...
> [!note] 中文学习笔记
> 中文题名：长时运行 Agent 的有效 Harness。阅读导图：作者把跨上下文窗口的长期开发视为“交接班”问题，依次用初始化环境、可验证的特性清单、增量提交与端到端测试建立可延续的工作状态。阅读时把它当作一套 harness 设计案例，而不是对所有 Agent 任务都已验证的通用定律。
>
> 相关：[[AI 与 Coding]]
...
<!-- codex-learning-note:start id="section-long-running-agent-problem" -->
> [!note] 中文学习笔记
> 论点：仅靠上下文压缩不足以让 Agent 持续完成复杂工程；会出现一次性铺开任务、上下文耗尽后无法交接，以及过早宣告完成等模式。证据来自作者对高阶提示下 Web 应用构建失败方式的观察。限制：案例主要针对编码任务，不能直接外推到所有长期知识工作。与引言的关系：本节定义了后续 harness 需要解决的状态连续性问题。
<!-- codex-learning-note:end -->
...
<!-- codex-learning-note:start id="section-environment-management" -->
> [!note] 中文学习笔记
> 本节把“记忆”落到环境产物，而非寄望模型自行记住上下文：初始化 Agent 负责留下未来会话可读取的项目状态。其证据是后续三个组件——特性表、增量进度和测试——共同降低了接手成本；这也是对上一节失败模式的工程化回应。
<!-- codex-learning-note:end -->
## Getting up to speed
...
<!-- codex-learning-note:start id="section-getting-up-to-speed" -->
> [!note] 中文学习笔记
> 阅读要点：这里的开局流程（定位目录、读提交与进度、选未完成特性、先做冒烟测试）是把交接产物转换成下一轮可执行决策的步骤。作者的推论是先确认基线健康，能避免把已有故障叠加到新功能上；它依赖于前节已经维护好 git、进度与测试资产。
<!-- codex-learning-note:end -->
## Future work
...
1\. We refer to these as separate agents in this context only because they have different initial user prompts. The system prompt, set of tools, and overall agent harness was otherwise identical.<!-- codex-learning-note:start id="section-future-work" -->
> [!note] 中文学习笔记
> 作者结论是这套方案展示了跨窗口增量推进的一种可行组合，而非最终架构。特别值得保留的开放问题是：通用编码 Agent 与按测试、质保、清理分工的多 Agent 架构何者更优，且现有演示仍主要面向全栈 Web 开发。
<!-- codex-learning-note:end -->
