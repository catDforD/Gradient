...
> [!note] 中文学习笔记
> 中文题名：托管 Agent 的扩展与脑手解耦。阅读导图：作者用模型能力变化导致旧 harness 假设失效这一现象，论证应把负责推理的“脑”与执行、状态和权限等“手”分离，并让系统能按任务扩缩。文章是面向托管 Agent 的工程设计讨论，阅读时要留意哪些结论依赖特定模型与平台。
>
> 相关：[[AI 与 Coding]]
...
<!-- codex-learning-note:start id="section-dont-adopt-a-pet" -->
> [!note] 中文学习笔记
> 本节的比喻提醒：不要把曾经有效的 Agent 行为假设永久固化在 harness 中。作者用新模型不再出现“上下文焦虑”、旧的重置逻辑反而成为负担的例子，说明应持续检验假设；限制是单一模型版本的行为变化仍需在自身工作负载中复测。
<!-- codex-learning-note:end -->
...
<!-- codex-learning-note:start id="section-decouple-brain-from-hands" -->
> [!note] 中文学习笔记
> 核心论点是把模型的高层判断与环境执行、工具、状态管理等职责拆开，使系统能在模型能力改变时替换或简化其中一侧。证据来自作者的托管 Agent 设计经验；与上一节相连：解耦减少了为补偿旧模型限制而写死的耦合逻辑。
<!-- codex-learning-note:end -->
...
<!-- codex-learning-note:start id="section-session-is-not-context-window" -->
> [!note] 中文学习笔记
> 这里区分产品层面的工作会话与单次模型上下文窗口：后者只是一次推理的容量边界，前者可由外部状态、工具和调度跨越多个窗口延续。该区分帮助理解为何“重开上下文”不等于丢失整个任务，也为后文的并行与托管执行铺路。
<!-- codex-learning-note:end -->
...
<!-- codex-learning-note:start id="section-many-brains-many-hands" -->
> [!note] 中文学习笔记
> 作者将解耦进一步推广到多模型或多执行单元的组合：不同“脑”和“手”可以按能力、成本与并发需求配置。要注意这不是天然收益；协调、权限边界、可观测性和失败恢复会随组合数增加而更重要，这也与多 Agent 系统的通用挑战相呼应。
<!-- codex-learning-note:end -->
## Conclusion
...
> [!note] 中文学习笔记
> 结论回到可演进的 harness：随着模型进步，应把过时的补偿机制移除或重构，并让推理与执行的边界保持清晰。作者的主张是设计原则和实践方向，不是某一固定架构的处方；采用前仍应以真实任务的评测与运维数据验证。
<!-- codex-learning-note:end -->
## Acknowledgements
